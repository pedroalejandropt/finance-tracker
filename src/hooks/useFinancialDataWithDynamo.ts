...
const dynamoClient = new DynamoDBClient({
    ...
    credentials: {
        accessKeyId: process.env.AWS_ACCESS_KEY_ID || 'REMOVED_ACCESS_KEY_ID',
        secretAccessKey: process.env.AWS_SECRET_ACCESS_KEY || 'REMOVED_SECRET_ACCESS_KEY',
    },
});
...
export function useFinancialDataWithDynamo() {
    ...
    const dynamoOperations = {
        ...
        createUser: async (user: Omit<DynamoDBUser, 'createdAt' | 'updatedAt'>): Promise<DynamoDBUser> => {
            try {
        ...
        batchSaveStocks: async (userId: string, stocks: Stock[]): Promise<void> => {
            try {
                ...
                await Promise.all(stocks.map(stock =>
                    dynamoOperations.saveStock(userId, stock)
                ));
            } catch (error) {
            ...
            try {
                ...
                try {
                  ...
                  if (rates.length === 0) {
                    ...
                    const currencyRateArray: CurrencyRate[] = Object.entries(apiRates.rates).map(([to, rate]) => ({
                      from: apiRates.base,
                      to: to,
                      rate,
                      timestamp: new Date()
                    ...
                    await Promise.all(currencyRateArray.map(rate => 
                      dynamoOperations.saveCurrencyRate(rate)
                    ));
